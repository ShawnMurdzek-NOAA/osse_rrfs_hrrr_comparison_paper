...
plt.subplots_adjust(left=0.09, bottom=0.21, right=0.99, top=0.99, hspace=0.1, wspace=0.42)
for i, (model, ls) in enumerate(zip(['RRFS', 'HRRR'], ['--', '-'])):
...
for i in range(2):
    for j, v in enumerate(plot_dict.keys()):
        ...
        # Subplot labels
        ax.text(0.84, 0.07, f'{letters[3*i+j]})', size=fontsize, weight='bold', transform=ax.transAxes,
                backgroundcolor='white')

        # Legend
        if (i == 1) and (j == 1):
            ax.legend(ncols=4, fontsize=fontsize, loc=(-1.75, -0.53))
        else:
